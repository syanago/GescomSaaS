    ...
    IInventoryService inventoryService) : CommercialPageModel(dbContext, currentTenantAccessor)
{
    ...
    public IReadOnlyList<InventorySerialItem> SerialNumbers { get; private set; } = [];

    public async Task OnGetAsync()
    {
        var tenantId = await GetTenantIdAsync();
        await LoadLookupsAsync(tenantId);
        SerialNumbers = await inventoryService.GetSerialStocksAsync(tenantId, ProductId, WarehouseId, HttpContext.RequestAborted);
    }
