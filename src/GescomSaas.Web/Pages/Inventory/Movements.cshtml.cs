    ...
    IInventoryService inventoryService) : CommercialPageModel(dbContext, currentTenantAccessor)
{
    ...
    public IReadOnlyList<InventoryMovementItem> Movements { get; private set; } = [];

    public async Task OnGetAsync()
    {
        var tenantId = await GetTenantIdAsync();
        await LoadLookupsAsync(tenantId);
        Movements = await inventoryService.GetMovementsAsync(tenantId, ProductId, WarehouseId, HttpContext.RequestAborted);
    }
