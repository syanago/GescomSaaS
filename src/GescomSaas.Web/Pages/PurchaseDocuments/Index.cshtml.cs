    ...
    ITenantQuotaEnforcementService tenantQuotaEnforcementService) : CommercialPermissionPageModel(dbContext, currentTenantAccessor, userPermissionService)
{
    ...
    public IReadOnlyList<PurchaseDocumentListItem> Documents { get; private set; } = [];
    ...
    {
        CurrentType = PurchaseDocumentCatalog.Normalize(Type);
        ...
        Documents = await query
            ...
            .Select(x => new PurchaseDocumentListItem(
                x.Id,
                x.Number,
                x.DocumentDate,
    ...
    public string Title => PurchaseDocumentCatalog.Label(CurrentType);
}

public sealed record PurchaseDocumentListItem(
    Guid Id,
    string Number,
